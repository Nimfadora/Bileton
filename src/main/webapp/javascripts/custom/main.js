    ...
    App.Views.AudienceView = Backbone.View.extend({
        ...
        choose: function(e){
            ...
            if($("#"+e.target.id).css("backgroundColor") == "rgb(255, 0, 0)")
            {
                ...
                this.arrOfPrices.splice(_.indexOf(this.arrOfPrices, this.model.get('price')[placeClass]), 1);

                if(this.arrOfPrices.length == 0)
                    disableNext();
                $('.ticketPrice').remove();
                $(".priceContainer").append('<p class="ticketPrice">Сумма:&nbsp&nbsp&nbsp&nbsp&nbsp'+ _.reduce(this.arrOfPrices, function(memo, num){ return memo + num; }, 0) +' грн</p>');
            }
            else
            {
                ...
                this.arrOfPrices.push(this.model.get('price')[placeClass]);
                $('.ticketPrice').remove();
                $(".priceContainer").append('<p class="ticketPrice">Сумма:&nbsp&nbsp&nbsp&nbsp&nbsp'+ _.reduce(this.arrOfPrices, function(memo, num){ return memo + num; }, 0) +' грн</p>');
                next();
            }
